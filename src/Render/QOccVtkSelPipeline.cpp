...
QOccVtkSelPipeline::QOccVtkSelPipeline(
	const TopoDS_Shape& theShape, const Standard_Integer theTag): _shape(theShape), _Tag(theTag)
{
    // 组织filter以供使用
    ...
    _dataSource->SetShape(new IVtkOCC_Shape(theShape));
    _id = _dataSource->GetId();
...
void QOccVtkSelPipeline::updateDataSource()
{
  ...
  * =========================== */
    ...
    _actor = vtkSmartPointer<vtkActor>::New();
    _dataSource = vtkSmartPointer<IVtkTools_ShapeDataSource>::New();
    _dataSource->SetShape(new IVtkOCC_Shape(_shape));
...
void QOccVtkSelPipeline::updateShape(const TopoDS_Shape &shape) {
    _actor = vtkSmartPointer<vtkActor>::New();
    _dataSource = vtkSmartPointer<IVtkTools_ShapeDataSource>::New();
    _dataSource->SetShape(new IVtkOCC_Shape(shape));
}

const TopoDS_Shape QOccVtkSelPipeline::getShape() const {
    return _shape;
}
