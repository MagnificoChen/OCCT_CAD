...
CentralViewWidget::CentralViewWidget(QWidget* parent)
    : QWidget(parent)
      ...
      , m_pickInteractorStyle(vtkSmartPointer<QOCCVTKIStyle>::New())
...
void CentralViewWidget::addPipelineToRenderer(Handle(QOccVtkSelPipeline) pipeline)
{
    ...
    m_pickInteractorStyle->addPipeline(pipeline);
    FitView();
}
...
void CentralViewWidget::OnSetSelectionMode(IVtk_SelectionMode mode)
{
    ...
    if (mode == SM_None)
    {
        ...
        m_pickInteractorStyle->ClearHiliAndSel();
...
{
    for( auto item: m_pickInteractorStyle->getPipelines())
    {
        //item->updatePrimaryPipeline();
    ...
    for (; iter.More(); iter.Next())
    {
        ...
        if(iter.Value().FindAttribute(TNaming_NamedShape::GetID(),hAttr)){
            if(hAttr->Evolution() == TNaming_DELETE) continue;
        }
        TopoDS_Shape shape;
