...
//    connect(m_toolbutton_delete, &QToolButton::toggled, this, &QOccPointSelector::OnDeleteButtonToggled);
    ...
    connect(this, &QOccGeoSelector::SelectModeChanged, ownerWidget, &CentralViewWidget::OnSetSelectionMode,Qt::DirectConnection);
...
QOccGeoSelector::~QOccGeoSelector() {
    ...
    qDebug()<<"QOccGeoSelector destructing...";
...
TopoDS_Shape QOccGeoSelector::getFullShape() {

    //仅在单选模式启用
    if(m_capacity!=1) return TopoDS_Shape();
...
void QOccGeoSelector::setFullShape(TopoDS_Shape &fullshape) {
    if(!m_fullShape.IsNull()){
        qDebug()<<"[WARNING] fullShape 之前不为空！";
        m_fullShape.Nullify();
    }
    m_fullShape = fullshape;
}
