...
#include "../Feature/FeatureFillet.h"
...
DialogFeatureChamfer::DialogFeatureChamfer(QWidget* parent)
    ...
    m_comboBox->addItems({"倒斜角", "倒圆角"});
    m_comboBox->setCurrentIndex(0);
    connect(m_comboBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
            &DialogFeatureChamfer::OnComboBoxChanged);
...
void DialogFeatureChamfer::OnOK() {
    if (!m_selectorEdge->isDataValid()) {
        done(QDialog::Rejected);
        return;
    } else {
        if (_curType == FILLET_CHAMFER) {
            auto fullshape = m_selectorEdge->getFullShape();
            auto solid = FeatureChamfer::FindParentSolidOrShell(m_selectorEdge->getSelectedEdge(), fullshape);
            FeatureChamfer feature(QDocumentManager::getInstance()->getCurrentDocument(),
                                   solid,
                                   TopoDS::Edge(m_selectorEdge->getSelectedShape()), 0.1);


            feature.AddToAttachList(m_selectorEdge->GetTag());
            feature.updateFeature();
            getOwnerWidget()->UpdateView();
        } else {
            auto solid = FeatureChamfer::FindParentSolidOrShell(m_selectorEdge->getSelectedEdge(),
                                                                m_selectorEdge->getFullShape());
            FeatureFillet feature(QDocumentManager::getInstance()->getCurrentDocument(),
                                  solid, TopoDS::Edge(m_selectorEdge->getSelectedShape()),
                                  0.1);
